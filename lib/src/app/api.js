...
router.get('/firewall', middleware.validator({
    ...
    if (req.data.filter.host) {
        filter['host'] = req.data.filter.host;
    }

    if (req.data.filter.action) {
        filter['action'] = req.data.filter.action;
    }

    if (req.data.filter.roles) {
        filter['roles'] = JSON.stringify(
            req.data.filter.roles
        );
    }

    const data = await database.select('firewall', filter, ['createdAt', 'DESC'], {
        page: req.data.page,
        limit: req.data.limit
    });

    const count = await database.count('firewall', filter);
